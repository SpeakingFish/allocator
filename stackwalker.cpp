...
BOOL StackWalker::GetCallStack(CallStack* stack, HANDLE hThread, CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  CONTEXT c;;
...
#error "Platform not supported!"
  ...
  for (frameNum = 0; ; ++frameNum )
  {
	...
	stack->push_back((DWORD_PTR)csEntry.offset);
  }
...
BOOL StackWalker::PrintCallStack(CallStack* stack, NMLeakCheckReport* report)
{
	CallstackEntry csEntry;
	IMAGEHLP_SYMBOL64 *pSym = NULL;
	StackWalkerInternal::IMAGEHLP_MODULE64_V2 Module;
	...
	for (CallStack::iterator iter = stack->begin(); iter != stack->end(); ++iter)
	{
		if (--skipFrames > 0)
		{
			continue;
		...
		if (csEntry.offset != 0)
		{
			// we seem to have a valid PC
			// show procedure info (SymGetSymFromAddr64())
			if (this->m_sw->pSGSFA(this->m_hProcess, csEntry.offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
			{
				// TODO: Mache dies sicher...!
				strcpy_s(csEntry.name, pSym->Name);
				// UnDecorateSymbolName()
			...
			if (this->m_sw->pSGLFA != NULL )
			{ // yes, we have SymGetLineFromAddr64()
				if (this->m_sw->pSGLFA(this->m_hProcess, csEntry.offset, &(csEntry.offsetFromLine), &Line) != FALSE)
				{
					csEntry.lineNumber = Line.LineNumber;
					// TODO: Mache dies sicher...!
					strcpy_s(csEntry.lineFileName, Line.FileName);
			...
			this->OnCallstackEntry(nextEntry, csEntry, report);
		} 
	}
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, NMLeakCheckReport* report)
{
  CHAR buffer[STACKWALK_MAX_NAMELEN];
